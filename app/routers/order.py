...
from app.schemas import OrderBase, OrderRead, OurBaseModelOut, PagedResponse, UserRead
...
@router.get("/")
def get_orders(db: DbSession = Depends(get_db), page: int = 1, page_size: int = 10, current_user: UserRead = Depends(get_current_user)):
    try:
