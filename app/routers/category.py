...
from app.schemas import CategoryCreate, CategoryRead, PagedResponse, OurBaseModelOut, CategoryUpdate, UserRead
...
@router.get("/")
def get_categories(db: Session = Depends(get_db), page: int = 1, page_size: int = 10, current_user: UserRead = Depends(get_current_user)):
    try:
